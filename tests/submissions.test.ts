...
describe("fetchSubmission", () => {
  it("GETs the correct URL with auth and decodes the response", async () => {
    let req: any;
    const client = makeTestHttpClient((r) => { req = r; return single(submissionFixture); });
  ...
  it("returns EdlinkApiError on 404, EdlinkDecodeError on bad schema", async () => {
    const err404 = await runFail(fetchSubmission(testConfig, makeTestHttpClient(() => fail(404)), CLS, ASGN, SUB));
    expect(err404).toBeInstanceOf(EdlinkApiError);

    const errDecode = await runFail(
      fetchSubmission(testConfig, makeTestHttpClient(() => single({ id: "x" })), CLS, ASGN, SUB),
    );
...
describe("submitAttempt", () => {
  ...
  it("returns EdlinkApiError on 400", async () => {
    const err = await runFail(submitAttempt(testConfig, makeTestHttpClient(() => fail(400)), CLS, ASGN, body));
...
describe("reclaimSubmission", () => {
  it("POSTs to the /reclaim URL with empty body", async () => {
    ...
    const client = makeTestHttpClient((r) => { req = r; return single(reclaimed); });
  ...
  it("returns EdlinkApiError on 404", async () => {
    const err = await runFail(reclaimSubmission(testConfig, makeTestHttpClient(() => fail(404)), CLS, ASGN));
    expect(err).toBeInstanceOf(EdlinkApiError);
  });
});
...
describe("returnSubmission", () => {
  it("POSTs to the /return URL with empty body", async () => {
    ...
    const client = makeTestHttpClient((r) => { req = r; return single(returned); });
  ...
  it("returns EdlinkApiError on 404", async () => {
    const err = await runFail(returnSubmission(testConfig, makeTestHttpClient(() => fail(404)), CLS, ASGN, SUB));
    expect(err).toBeInstanceOf(EdlinkApiError);
  });
});
...
describe("updateSubmission", () => {
  ...
  it("returns EdlinkApiError on 404", async () => {
    const err = await runFail(updateSubmission(testConfig, makeTestHttpClient(() => fail(404)), CLS, ASGN, SUB, patch));
    expect(err).toBeInstanceOf(EdlinkApiError);
  });
});
...
describe("listSubmissions", () => {
  it("streams items across pages; returns empty for no data", async () => {
    const empty = await collect(
      listSubmissions(testConfig, makeTestHttpClient(() => page([])), CLS, ASGN, { type: "all" }),
    );
    expect(empty).toHaveLength(0);

    const items = await collect(
      listSubmissions(testConfig, makeTestHttpClient(() => page([submissionFixture, submissionFixture2])), CLS, ASGN, { type: "all" }),
    );
    ...
    const multiItems = await collect(listSubmissions(testConfig, makeTestHttpClient(multi), CLS, ASGN, { type: "all" }));
  ...
  it("respects maxPages and maxRecords pagination limits", async () => {
    ...
    const byPages = await collect(
      listSubmissions(testConfig, makeTestHttpClient(() => {
        pc++;
        return page([{ ...submissionFixture, id: `s-${pc}` }], `${BASE}/next?p=${pc + 1}`);
      }), CLS, ASGN, { type: "pages", maxPages: 2 }),
    );
    expect(pc).toBe(2);
    ...
    const byRecs = await collect(
      listSubmissions(testConfig, makeTestHttpClient(() => {
        rc++;
        return page(
          [{ ...submissionFixture, id: `r-${rc}a` }, { ...submissionFixture2, id: `r-${rc}b` }, { ...submissionFixture3, id: `r-${rc}c` }],
          `${BASE}/next?p=${rc + 1}`,
        );
      }), CLS, ASGN, { type: "records", maxRecords: 5 }),
    );
    expect(rc).toBe(2);
  ...
  it("returns EdlinkApiError on 500, EdlinkDecodeError on bad data", async () => {
    const err500 = await collectFail(
      listSubmissions(testConfig, makeTestHttpClient(() => fail(500)), CLS, ASGN, { type: "all" }),
    );
    expect(err500).toBeInstanceOf(EdlinkApiError);

    const errDecode = await collectFail(
      listSubmissions(testConfig, makeTestHttpClient(() => page([{ id: "bad" }])), CLS, ASGN, { type: "all" }),
    );
    expect(errDecode).toBeInstanceOf(EdlinkDecodeError);
  });
});
