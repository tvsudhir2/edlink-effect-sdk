...
describe("createEventsStream", () => {
  it("streams events across pages; returns empty for no data", async () => {
    const empty = await collect(
      createEventsStream(testConfig, makeTestHttpClient(() => page([])), { type: "all" }),
    );
    expect(empty).toHaveLength(0);

    const items = await collect(
      createEventsStream(testConfig, makeTestHttpClient(() => page([eventFixture, eventFixture2])), { type: "all" }),
    );
  ...
  it("respects maxPages and maxRecords pagination limits", async () => {
    ...
    const byPages = await collect(
      createEventsStream(testConfig, makeTestHttpClient(() => {
        pc++;
    ...
    const byRecs = await collect(
      createEventsStream(testConfig, makeTestHttpClient(() => {
        rc++;
        return page(
          [{ ...eventFixture, id: `r-${rc}a` }, { ...eventFixture2, id: `r-${rc}b` }, { ...eventFixture3, id: `r-${rc}c` }],
          `${BASE}/next?p=${rc + 1}`,
        );
  ...
  it("returns EdlinkApiError on 500, EdlinkDecodeError on bad data", async () => {
    const err500 = await collectFail(
      createEventsStream(testConfig, makeTestHttpClient(() => fail(500)), { type: "all" }),
    );
    ...
    const errDecode = await collectFail(
      createEventsStream(testConfig, makeTestHttpClient(() => page([{ noId: true }])), { type: "all" }),
    );
