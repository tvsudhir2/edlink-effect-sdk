...
import { listEnrollments, fetchEnrollment } from "../src/api/v2/enrollments.js";
...
describe("fetchEnrollment", () => {
  it("GETs the correct URL with auth and decodes the response", async () => {
    let req: any;
    const client = makeTestHttpClient((r) => { req = r; return single(enrollmentFixture); });
  ...
  it("returns EdlinkApiError on 404, EdlinkDecodeError on bad schema", async () => {
    const err404 = await runFail(fetchEnrollment(testConfig, makeTestHttpClient(() => fail(404)), ENR));
    expect(err404).toBeInstanceOf(EdlinkApiError);

    const errDecode = await runFail(
      fetchEnrollment(testConfig, makeTestHttpClient(() => single({ id: "x" })), ENR),
    );
...
describe("listEnrollments", () => {
  it("streams items across pages", async () => {
    const empty = await collect(
      listEnrollments(testConfig, makeTestHttpClient(() => page([])), { type: "all" }),
    );
  ...
  it("respects maxPages and maxRecords", async () => {
    ...
    const byPages = await collect(
      listEnrollments(testConfig, makeTestHttpClient(() => {
        pc++;
    ...
    const byRecs = await collect(
      listEnrollments(testConfig, makeTestHttpClient(() => {
        rc++;
        return page(
          [{ ...enrollmentFixture, id: `r-${rc}a` }, { ...enrollmentFixture2, id: `r-${rc}b` }, { ...enrollmentFixture3, id: `r-${rc}c` }],
          `${BASE}/next?p=${rc + 1}`,
        );
  ...
  it("returns EdlinkApiError on 500, EdlinkDecodeError on bad data", async () => {
    const err = await collectFail(listEnrollments(testConfig, makeTestHttpClient(() => fail(500)), { type: "all" }));
    expect(err).toBeInstanceOf(EdlinkApiError);

    const errDecode = await collectFail(
      listEnrollments(testConfig, makeTestHttpClient(() => page([{ bad: true }])), { type: "all" }),
    );
    expect(errDecode).toBeInstanceOf(EdlinkDecodeError);
  });
});
