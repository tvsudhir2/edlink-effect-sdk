...
import { listCourses, fetchCourse, listCourseClasses } from "../src/api/v2/courses.js";
...
import { courseFixture, courseFixture2, courseFixture3, classFixture, classFixture2 } from "./helpers/fixtures.js";
...
describe("fetchCourse", () => {
  it("GETs the correct URL with auth and decodes the response", async () => {
    let req: any;
    const client = makeTestHttpClient((r) => { req = r; return single(courseFixture); });
  ...
  it("returns EdlinkApiError on 404, EdlinkDecodeError on bad schema", async () => {
    const err404 = await runFail(fetchCourse(testConfig, makeTestHttpClient(() => fail(404)), CRS));
    expect(err404).toBeInstanceOf(EdlinkApiError);

    const errDecode = await runFail(
      fetchCourse(testConfig, makeTestHttpClient(() => single({ id: "x" })), CRS),
    );
...
describe("listCourses", () => {
  it("streams items across pages", async () => {
    const empty = await collect(
      listCourses(testConfig, makeTestHttpClient(() => page([])), { type: "all" }),
    );
  ...
  it("respects maxPages and maxRecords", async () => {
    ...
    const byPages = await collect(
      listCourses(testConfig, makeTestHttpClient(() => {
        pc++;
    ...
    const byRecs = await collect(
      listCourses(testConfig, makeTestHttpClient(() => {
        rc++;
        return page(
          [{ ...courseFixture, id: `r-${rc}a` }, { ...courseFixture2, id: `r-${rc}b` }, { ...courseFixture3, id: `r-${rc}c` }],
          `${BASE}/next?p=${rc + 1}`,
        );
  ...
  it("returns EdlinkApiError on 500, EdlinkDecodeError on bad data", async () => {
    const err = await collectFail(listCourses(testConfig, makeTestHttpClient(() => fail(500)), { type: "all" }));
    expect(err).toBeInstanceOf(EdlinkApiError);

    const errDecode = await collectFail(
      listCourses(testConfig, makeTestHttpClient(() => page([{ id: "bad" }])), { type: "all" }),
    );
    expect(errDecode).toBeInstanceOf(EdlinkDecodeError);
  });
});
...
describe("listCourseClasses", () => {
  it("streams classes for a course", async () => {
    ...
    const items = await collect(
      listCourseClasses(testConfig, makeTestHttpClient((r) => { req = r; return page([classFixture, classFixture2]); }), CRS, { type: "all" }),
    );
