...
describe("fetchCategory", () => {
  it("GETs the correct URL with auth and decodes the response", async () => {
    let req: any;
    const client = makeTestHttpClient((r) => { req = r; return single(categoryFixture); });
  ...
  it("returns EdlinkApiError on 404, EdlinkDecodeError on bad schema", async () => {
    const err404 = await runFail(fetchCategory(testConfig, makeTestHttpClient(() => fail(404)), CLS, CAT));
    expect(err404).toBeInstanceOf(EdlinkApiError);

    const errDecode = await runFail(
      fetchCategory(testConfig, makeTestHttpClient(() => single({ id: "x" })), CLS, CAT),
    );
...
describe("createCategory", () => {
  ...
  it("returns EdlinkApiError on 400, EdlinkDecodeError on bad schema", async () => {
    const err400 = await runFail(createCategory(testConfig, makeTestHttpClient(() => fail(400)), CLS, body));
    expect(err400).toBeInstanceOf(EdlinkApiError);

    const errDecode = await runFail(
      createCategory(testConfig, makeTestHttpClient(() => single({ id: "x" })), CLS, body),
    );
    expect(errDecode).toBeInstanceOf(EdlinkDecodeError);
  });
});
...
describe("updateCategory", () => {
  ...
  it("returns EdlinkApiError on 404", async () => {
    const err = await runFail(updateCategory(testConfig, makeTestHttpClient(() => fail(404)), CLS, CAT, patch));
...
describe("deleteCategory", () => {
  ...
  it("returns EdlinkApiError on 404", async () => {
    const err = await runFail(deleteCategory(testConfig, makeTestHttpClient(() => fail(404)), CLS, CAT));
    expect(err).toBeInstanceOf(EdlinkApiError);
  });
});
...
describe("listCategories", () => {
  it("streams items across pages; returns empty for no data", async () => {
    const empty = await collect(
      listCategories(testConfig, makeTestHttpClient(() => page([])), CLS, { type: "all" }),
    );
    expect(empty).toHaveLength(0);

    const items = await collect(
      listCategories(testConfig, makeTestHttpClient(() => page([categoryFixture, categoryFixture2])), CLS, { type: "all" }),
    );
  ...
  it("respects maxPages and maxRecords pagination limits", async () => {
    ...
    const byPages = await collect(
      listCategories(testConfig, makeTestHttpClient(() => {
        pc++;
    ...
    const byRecs = await collect(
      listCategories(testConfig, makeTestHttpClient(() => {
        rc++;
        return page(
          [{ ...categoryFixture, id: `r-${rc}a` }, { ...categoryFixture2, id: `r-${rc}b` }, { ...categoryFixture3, id: `r-${rc}c` }],
          `${BASE}/next?p=${rc + 1}`,
        );
  ...
  it("returns EdlinkApiError on 500, EdlinkDecodeError on bad data", async () => {
    const err500 = await collectFail(
      listCategories(testConfig, makeTestHttpClient(() => fail(500)), CLS, { type: "all" }),
    );
    expect(err500).toBeInstanceOf(EdlinkApiError);

    const errDecode = await collectFail(
      listCategories(testConfig, makeTestHttpClient(() => page([{ id: "bad" }])), CLS, { type: "all" }),
    );
    expect(errDecode).toBeInstanceOf(EdlinkDecodeError);
  });
});
