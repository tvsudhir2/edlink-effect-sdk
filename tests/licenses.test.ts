...
describe("listLicenses", () => {
  it("streams items across pages; returns empty for no data", async () => {
    const empty = await collect(
      listLicenses(testConfig, makeTestHttpClient(() => page([])), { type: "all" }),
    );
    expect(empty).toHaveLength(0);

    const items = await collect(
      listLicenses(testConfig, makeTestHttpClient(() => page([licenseFixture, licenseFixture2])), { type: "all" }),
    );
  ...
  it("respects maxPages and maxRecords pagination limits", async () => {
    ...
    const byPages = await collect(
      listLicenses(testConfig, makeTestHttpClient(() => {
        pc++;
    ...
    const byRecs = await collect(
      listLicenses(testConfig, makeTestHttpClient(() => {
        rc++;
        return page(
          [{ ...licenseFixture, product_id: `r-${rc}a` }, { ...licenseFixture2, product_id: `r-${rc}b` }, { ...licenseFixture3, product_id: `r-${rc}c` }],
          `${BASE}/next?p=${rc + 1}`,
        );
  ...
  it("returns EdlinkApiError on 500, EdlinkDecodeError on bad data", async () => {
    const err500 = await collectFail(
      listLicenses(testConfig, makeTestHttpClient(() => fail(500)), { type: "all" }),
    );
    expect(err500).toBeInstanceOf(EdlinkApiError);

    const errDecode = await collectFail(
      listLicenses(testConfig, makeTestHttpClient(() => page([{ bad: true }])), { type: "all" }),
    );
